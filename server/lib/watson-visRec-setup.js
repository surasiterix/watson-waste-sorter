    ...
    this.vizRecClient.listClassifiers({}, (err,response) => {
      if (err) {
        ...
        for (let i = 0, size = classifiers.length; i < size; i++) {
          ...
          if (classifier.name === 'wasteSorterAnalyzer') {
...
WatsonVisRecSetup.prototype.createVisRecClassifier = function(params) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var createClassifierParams = {
        name: 'wasteSorterAnalyzer',
        Compost_positive_examples: fs.createReadStream('./data/compost.zip'),
        Landfill_positive_examples: fs.createReadStream('./data/landfill.zip'),
        Recycle_positive_examples: fs.createReadStream('./data/recycle.zip'),
        negative_examples: fs.createReadStream('./data/negative.zip')
      }
